...
def _get_sim112(node: ast.Expr) -> List[Tuple[int, int, str]]:
    ...
    is_index_call = (
        ...
        and isinstance(node.value.slice.value, AST_CONST_TYPES)
    )
    if is_index_call:
        ...
        string_part = slice.value
        assert isinstance(string_part, AST_CONST_TYPES)
        env_name: str = string_part.value
        # Check if this has a change
        has_change = string_part.value != string_part.value.upper()
    ...
    is_get_call = (
        ...
        and isinstance(node.value.args[0], AST_CONST_TYPES)
    )
    if is_get_call:
        call = node.value
    ...
    if is_index_call:
        ...
        if len(node.value.args) == 1:  # type: ignore
            ...
            expected = f'os.environ.get("{env_name.upper()}", "{node.value.args[1].value}")'  # type: ignore
    else:
        return errors
    errors.append(
        (
